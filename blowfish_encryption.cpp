...
long long modVal = pow(2, 32);

//public functions
string blowfishEncrypt(string plainText, string key);
string blowfishDecrypt(string encryptedText, string key);
string blowfishGenerateKey();
bool blowfishCheckKey(string key);
bool blowfishCheckText(string text);

//private functions
string hexToBin(string hexValue);
string binToHex(string binValue);
string XOR(string a, string b);
string addBin(string a, string b);
string f(string value);
string* generateSubkeys(string key);
string round(int i, string value, string* subKeys);
string encrypt(string value, string* subKeys);
string decrypt(string value, string* subKeys);
...
string binToHexChar(string bin)
{
...
string hexToBin(string hexValue)
{
    ...
    {
        binary += hexCharToBin(hexValue[i]);;
    }
    return binary;
...
{
    string hexValue = "";
    for(int i = 0; i < binValue.size(); i+=4)
    {
        hexValue += binToHexChar(binValue.substr(i, 4));
    }
    return hexValue;
}
...
{
    string result;
    long long nA = stoll(a, nullptr, 16);
    long long nB = stoll(b, nullptr, 16);
    long long n = (nA + nB) % modVal;
    ostringstream ss;
    ss << std::hex << n;
    result = "00000000" + ss.str();
...
string f(string value)
{
    ...
    for(int i = 0; i < 8; i += 2)
    {
        long n = stol(hexToBin(value.substr(i, 2)),nullptr, 2);
        a[i/2] = Sbox[i/2][n];
    }

    result = addBin(a[0], a[1]);
    result = XOR(result, a[2]);
    return addBin(result, a[3]);
}

string* generateSubkeys(string key)
{
    int j = 0;
    string* subKeys = new string[18];
    for(int i = 0; i < 18; i++)
    {
        subKeys[i] = XOR(P[i], key.substr(j, 8)); // 8
        j = (j + 8) % key.size();
    }
    return subKeys;
}
...
string round(int i, string value, string* subKeys)
{
    string left = value.substr(0, 8);
    string right = value.substr(8, 16);
    left = XOR(left, subKeys[i]);
    string fResult = f(left);
    right = XOR(fResult, right);
    return right + left;
}
...
string encrypt(string value, string* subKeys)
{
    for(int i = 0; i < 16; i++)
    {
        value = round(i, value, subKeys);
    }
    //swap
    string right = value.substr(0, 8);
    string left = value.substr(8, 16);
...
bool blowfishCheckKey(string key)
{
    // key has to be 32-bits - 448-bits each hexidecimal character is 4 bits
    if(key.length() < 8 || key.length() > 112) return false;
...
bool blowfishCheckText(string text)
{
    if(text.size()%16 != 0) return false;
...
string blowfishGenerateKey()
{
    ...
    int length = rand()%4 * 8 + 8;
    string key = "";
    for(int i = 0; i < length; i++)
    {
        key += hexValues[rand() % hexValues.size()];
    }
    return key;
}
...
string blowfishEncrypt(string plainText, string key)
{
    ...
    // 1 block is 64 bits long = 16 hexidecimal characters
    for(int i = 0; i < plainText.size(); i += 16)
    {
        cipher += encrypt(plainText.substr(i, 16), subKeys);
    }
    return cipher;
}
...
string blowfishDecrypt(string encryptedText, string key)
{
    ...
    {
        text += decrypt(encryptedText.substr(i, 16), subKeys);
    }
    return text;
}
